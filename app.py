...
if not os.path.exists(LETTERS_FOLDER):
    st.error(f"Folder {LETTERS_FOLDER} not found. Please upload letter PNG files.")
    st.stop()
...
letter_height_mm = st.sidebar.number_input("Letter height (mm)", min_value=10, max_value=200, value=100)
...
if st.button("Generate PDF"):
    ...
    if not lines:
        ...
        if missing_chars:
            st.warning(f"Missing character PNG files: {', '.join(sorted(missing_chars))}")
